...
function createTileMap() {
  ...
  document.addEventListener("click", event => {
    ...
    if(typeValue==='Secret'){
      let select3:HTMLSelectElement = selectFactory('secretType',[['Bombable','bombable'],['Burnable','burnable'],['Flute ']])
    }
    let shapeCode: string = shape.value;
    let currentSet = sets[setsString.indexOf(typeValue)] as Set<any>;
    for (let i = 0; i < map.length; i++) {
      if (
        event.clientX >= map[i][0] &&
        ...
        if (
          currentSet.has(
            JSON.stringify([map[i][0], map[i][1], 32, 34, shapeCode])
          )
        ) {
          currentSet.delete(
            JSON.stringify([map[i][0], map[i][1], 32, 34, shapeCode])
